<div align="center">

<img src="https://capsule-render.vercel.app/api?type=waving&color=0:0d1117,50:ff3ea5,100:58a6ff&height=170&section=header&text=Danya&fontSize=52&fontColor=ffffff&animation=fadeIn&fontAlignY=35&desc=Python%20%2F%20JavaScript%20Developer&descAlignY=58&descSize=18" />

<br />

<img src="https://readme-typing-svg.demolab.com?font=JetBrains+Mono&weight=600&size=22&duration=2600&pause=900&color=FF3EA5&center=true&vCenter=true&width=700&lines=hey%2C+i'm+Danya;17+y.o.+developer+from+Warsaw;building+with+Python+and+JavaScript;learning%2C+coding%2C+improving" alt="Typing SVG" />

</div>

---

<div align="center">

## tech stack

<img src="https://skillicons.dev/icons?i=python,js,nodejs,react,html,css,git,github,vscode" alt="Tech stack" />

</div>

---

## about me

```js
const danya = {
  age: 17,
  location: "Warsaw, Poland",
  code: ["Python", "JavaScript"],
  interests: ["web apps", "bots", "automation", "backend"],
  currentlyLearning: ["React", "Node.js", "better architecture"],
  goal: "turning ideas into clean and useful projects"
};
```

---

<div align="center">

## github stats

<img height="165" src="https://github-readme-stats.vercel.app/api?username=YOUR_USERNAME&show_icons=true&theme=radical&hide_border=true&bg_color=0d1117&title_color=ff3ea5&icon_color=58a6ff&text_color=c9d1d9" alt="GitHub stats" />
<img height="165" src="https://github-readme-stats.vercel.app/api/top-langs/?username=YOUR_USERNAME&layout=compact&theme=radical&hide_border=true&bg_color=0d1117&title_color=ff3ea5&text_color=c9d1d9" alt="Top languages" />

<br />
<br />

<img src="https://streak-stats.demolab.com?user=YOUR_USERNAME&theme=radical&hide_border=true&background=0D1117&ring=FF3EA5&fire=FF3EA5&currStreakLabel=FF3EA5&sideNums=58A6FF&currStreakNum=58A6FF" alt="GitHub streak" />

</div>

---

## projects

| project | description | stack |
|---|---|---|
| coming soon | a useful Python project is loading | Python |
| coming soon | web idea in progress | JavaScript / Node.js |
| coming soon | something experimental | Python / JS |

---

<div align="center">

## connect

<a href="https://github.com/TrueERD">
  <img src="https://img.shields.io/badge/GitHub-0D1117?style=for-the-badge&logo=github&logoColor=white" alt="GitHub" />
</a>
<a href="#">
  <img src="https://img.shields.io/badge/Telegram-0D1117?style=for-the-badge&logo=telegram&logoColor=26A5E4" alt="Telegram" />
</a>
<a href="#">
  <img src="https://img.shields.io/badge/Discord-0D1117?style=for-the-badge&logo=discord&logoColor=5865F2" alt="Discord" />
</a>

<br />
<br />

<img src="https://komarev.com/ghpvc/?username=TrueERD&style=for-the-badge&color=ff3ea5&label=profile+views" alt="Profile views" />

</div>

---

<div align="center">

```txt
code. learn. build. repeat.
```

</div>
